...
// --- HELPER: UUID (ROBUST) ---
const uuid = () => {
  // funktioniert auf localhost/https
  if (globalThis.crypto?.randomUUID) return globalThis.crypto.randomUUID();

  // Fallback (funktioniert überall)
...
// Definiert Regeln für Umbrüche und A4-Container
const PrintStyles = () => (
  <style>{`
    /* A4 Definition */
    @page { size: A4; margin: 10mm; }
    
    /* Hilfsklassen für Umbrüche */
    .pdf-avoid-break {
      page-break-inside: avoid !important;
      break-inside: avoid !important;
    }
    
    .pdf-always-break-before {
      page-break-before: always !important;
      break-before: always !important;
    }

    /* Container für den PDF-Generator (versteckt) */
    #pdf-print-container {
      width: 210mm; /* Exakt A4 Breite */
      min-height: 297mm;
      background-color: white;
      padding: 15mm; /* Etwas Rand für den Inhalt */
      box-sizing: border-box;
      position: absolute;
      top: 0;
      left: -9999px; /* Offscreen rendern */
      z-index: -10;
      font-family: 'Times New Roman', serif;
      color: black;
      font-size: 11pt; /* Lesbare Größe */
      line-height: 1.4;
    }

    /* Bilder im PDF begrenzen */
...
const ImageUploadButton = ({ image, onUpload, onRemove, labelSuffix = "Foto" }) => {
    ...
    return (
      <div className="relative inline-block mt-2">
        <div className="h-20 w-20 rounded-lg overflow-hidden border border-slate-300 relative group">
          ...
          <button onClick={onRemove} className="absolute top-0 right-0 bg-red-500 text-white p-1 rounded-bl-lg opacity-0 group-hover:opacity-100 transition-opacity" title="Bild entfernen">
            <X size={12} />
          </button>
        </div>
        <div className="text-[10px] text-slate-500 truncate max-w-[80px] mt-1 flex items-center gap-1">
          <Check size={10} className="text-green-500" /> Gespeichert
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mt-2">
      ...
      <button onClick={() => fileInputRef.current.click()} className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-300 rounded-lg text-slate-600 text-xs font-semibold hover:bg-slate-50 hover:text-blue-600 transition-colors">
...
const ProtocolContent = ({ data, signatures }) => {
  ...
  data.meters.main.forEach(m => {
    if(m.image) allImages.push({ title: m.imageName, src: m.image });
  });
  if(data.meters.hasHeating) {
    data.meters.heating.forEach(h => {
      if(h.image) allImages.push({ title: h.imageName || `HKV ${h.room}.jpg`, src: h.image });
    });
  }
  if(data.defects.hasDefects) {
    data.defects.list.forEach(d => {
      if(d.image) allImages.push({ title: d.imageName || `Mangel ${d.location}.jpg`, src: d.image });
    });
  }
  ...
  return (
    <div className="text-slate-900 font-serif text-sm leading-relaxed">
       {/* Header */}
       <div className="flex justify-between items-start mb-8 border-b-2 border-slate-800 pb-4 pdf-avoid-break">
        <div>
        ...
        <div className="text-right font-bold text-slate-700 border-l-4 border-blue-600 pl-3">
          LUDWIGS<br/>IMMOBILIEN
        </div>
      </div>

      {/* Meta Data - Zusammenhalten */}
      <div className="grid grid-cols-2 gap-8 mb-8 pdf-avoid-break">
          ...
          <div className="border-b border-slate-300 pb-1 min-h-[1.5em]">
             {data.meta.sellers.length > 0 && data.meta.sellers[0].name ? (
               <ul className="list-none m-0 p-0">
                 {data.meta.sellers.map(s => s.name && (
                   <li key={s.id}>
                     {s.name} 
                     {s.email && <span className="text-slate-500 text-xs italic ml-1">({s.email})</span>}
                   </li>
                 ))}
               </ul>
             ) : '—'}
        ...
        <div className="col-span-2 md:col-span-1">
           ...
           <div className="border-b border-slate-300 pb-1 min-h-[1.5em]">
             {data.meta.buyers.length > 0 && data.meta.buyers[0].name ? (
               <ul className="list-none m-0 p-0">
                 {data.meta.buyers.map(b => b.name && (
                   <li key={b.id}>
                     {b.name}
                     {b.email && <span className="text-slate-500 text-xs italic ml-1">({b.email})</span>}
                   </li>
                 ))}
               </ul>
             ) : '—'}
      ...
      {/* Keys Section - Ganze Tabelle zusammenhalten */}
            ...
            <tbody>
              {activeKeys.map((k, i) => (
                <tr key={i} className="border-b border-slate-100">
                  <td className="py-2">{k.type}</td>
                  <td className="py-2 font-mono font-bold">{k.count}</td>
                  <td className="py-2 font-mono text-slate-600">{k.number || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Meters Section */}
      <div className="mb-8">
        <h3 className="font-bold bg-slate-100 p-2 mb-2 border-l-4 border-slate-400 pdf-avoid-break">2. Zählerstände</h3>
        
        {/* Hauptzähler Grid - Items einzeln schützen */}
        {data.meters.main.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            {data.meters.main.map((m) => (
              <div key={m.id} className="border border-slate-300 p-3 rounded bg-white pdf-avoid-break">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="font-mono text-right">{m.number || '—'}</div>
                  ...
                  {m.image && (
                     <div className="col-span-2 mt-1 text-[10px] text-slate-500 italic">
                        [Siehe Anhang: {m.imageName}]
                     </div>
                  )}
                </div>
              </div>
          ...
          <p className="text-slate-400 italic mb-4">Keine Hauptzähler erfasst.</p>
        )}

        {/* Heizung - Tabelle zusammenhalten */}
        {showHeating && (
          <div className="mt-4 pdf-avoid-break">
              ...
              <tbody>
                {data.meters.heating.filter(h => h.number || h.reading).map((h, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-2 border border-slate-300 font-mono">{h.number}</td>
                    <td className="p-2 border border-slate-300 font-mono text-right font-bold">{h.reading}</td>
                    <td className="p-2 border border-slate-300 text-center text-[10px] italic text-slate-500">
                      {h.image ? "Ja (Anhang)" : "-"}
                    </td>
                  </tr>
                ))}
      ...
      {/* Defects Section */}
      <div className="mb-8">
        <h3 className="font-bold bg-slate-100 p-2 mb-2 border-l-4 border-red-400 text-red-900 pdf-avoid-break">3. Festgestellte Mängel</h3>
        {!showDefects ? (
           <p className="text-slate-500 italic px-2">Keine Mängel festgestellt.</p>
        ) : (
          <div className="space-y-4">
            {data.defects.list.map((defect, i) => (
              <div key={i} className="border border-red-100 bg-red-50/30 p-3 rounded pdf-avoid-break">
                 <div className="flex flex-col gap-1">
                    <p className="text-xs font-bold uppercase text-red-800">{defect.location || 'Ohne Ortsangabe'}</p>
                    <p className="text-sm text-slate-800">{defect.description || 'Keine Beschreibung'}</p>
                    {defect.image && (
                       <p className="text-[10px] text-slate-500 italic mt-1">[Foto im Anhang: {defect.imageName}]</p>
                    )}
                 </div>
              </div>
            ))}
      ...
      <div className="mb-8 pdf-avoid-break">
        ...
        <div className="mb-4">
          ...
          <p className="whitespace-pre-wrap">{data.docs || '—'}</p>
        </div>
        <div>
          <p className="text-xs uppercase text-slate-500 font-bold">Sonstige Bemerkungen</p>
          <p className="whitespace-pre-wrap">{data.remarks || 'Keine weiteren Bemerkungen.'}</p>
        </div>
      </div>

      {/* Signatures */}
      <div className="mt-12 pt-4 border-t-2 border-slate-900 pdf-avoid-break">
        <h3 className="font-bold uppercase tracking-widest mb-6 text-center">Unterschriften</h3>
        {signatures.length === 0 ? (
           <p className="text-center text-slate-400 italic py-8 border-2 border-dashed border-slate-200 bg-slate-50 rounded">
             Noch keine Unterschriften erfasst.
           </p>
        ) : (
      ...
      {/* Anhang */}
      {allImages.length > 0 && (
        <div className="mt-8 pt-8 border-t-4 border-slate-100 pdf-always-break-before">
           <h2 className="text-xl font-bold uppercase tracking-widest text-slate-900 mb-6">Anhang: Fotodokumentation</h2>
           <div className="grid grid-cols-2 gap-6">
             {allImages.map((img, idx) => (
               <div key={idx} className="border border-slate-200 p-2 rounded pdf-avoid-break">
...
const ProtocolDocument = ({ data, signatures }) => {
  return (
    <div className="bg-white shadow-lg border border-slate-200 mx-auto max-w-[210mm] p-[10mm]">
      <ProtocolContent data={data} signatures={signatures} />
    </div>
  );
};
...
export default function App() {
  ...
  // --- PDF GENERATION LOGIC (OPTIMIERT) ---
  
  const generatePdfBlob = async () => {
    const input = document.getElementById('pdf-print-container');
    
    if (!window.html2canvas || !window.jspdf) {
        throw new Error("PDF-Bibliotheken sind noch nicht geladen. Bitte kurz warten und erneut versuchen.");
    }

    try {
      // 1. Capture mit optimierten Einstellungen
      const canvas = await window.html2canvas(input, {
        scale: 2, // Gute Qualität für Text, Dateigröße wird durch JPEG reduziert
        useCORS: true,
        logging: false,
        windowWidth: 794, // Force A4 Context
        onclone: (clonedDoc) => {
            // Optional: Hier könnte man CSS Manipulationen am Klon vornehmen
            const element = clonedDoc.getElementById('pdf-print-container');
            if(element) element.style.display = 'block';
        }
      });

      // 2. WICHTIG: JPEG statt PNG nutzen für massive Platzersparnis (Qualität 0.7)
      ...
      // Erste Seite
      pdf.addImage(imgData, 'JPEG', 0, position, imgWidth, imgHeight); // JPEG Format
    ...
    switch(step) {
        ...
        <div className="space-y-6 animate-in fade-in">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-4">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               ...
               <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Anschrift</label><input placeholder="Musterstraße 12" value={data.meta.address} onChange={e => updateField('meta.address', e.target.value)} className="w-full p-3 border border-slate-300 rounded-lg outline-none" /></div>
             </div>
      ...
      case 3: return (
        <div className="flex flex-col lg:flex-row gap-8 animate-in fade-in">
          ...
          <div className="lg:w-2/3 order-1 lg:order-2"><div className="bg-slate-200 p-4 rounded-xl overflow-auto"><div className="mb-4 flex items-center gap-2 text-slate-600 font-bold"><Eye size={20} /> Protokoll-Vorschau</div><ProtocolDocument data={data} signatures={data.signatures} /></div></div>
        </div>
      );
      default: return null;
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-24">
      ...
      <div id="pdf-print-container">
        <ProtocolContent data={data} signatures={data.signatures} />
      </div>
